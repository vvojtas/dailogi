...
REM start "Dailogi Backend (Local+E2E)" cmd /c "mvn spring-boot:run -Dspring-boot.run.profiles=local,e2e-test
...
for /f "delims=" %%i in ('curl -s -o NUL -w "%%{http_code}" http://localhost:8080/actuator/health') do set HTTP_STATUS=%%i
...
REM Find and kill any process listening on port 8080 (fallback)
for /F "tokens=5" %%P in ('netstat -ano ^| findstr /R /C:"TCP.*:8080 " ^| findstr "LISTENING"') do (
    echo Killing process listening on port 8080: %%P
    taskkill /F /PID %%P > nul 2>&1
)
